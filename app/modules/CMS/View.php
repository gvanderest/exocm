...
{ 
	...
	public function render($template, $data = NULL)
	{
		...
		for ($x = 0; $x < 3; $x++)
		{
			$output = $this->library->parse_page_tags($this->application, $output);
		}
	...
	{
		include($this->theme_path . '/' . $path);
	}

	public function import_once($path)
	{
		include_once($this->theme_path . '/' . $path);
	}
}
